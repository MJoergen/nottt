...
Texture::Texture(const std::string text, SDL_Color textColor){
	
	SDL_Surface* textSurface = TTF_RenderText_Solid(g_font, text.c_str(), textColor);
	if (textSurface == NULL){
...
void Texture::renderTexture(const int x, const int y, const int width, const int height) const {
	...
	SDL_RenderCopy(g_renderer, m_texture, NULL, &dstRect);
}
...
void Texture::renderTexture(const int sx, const int sy, const int swidth, const int sheight, const int dx, const int dy, const int dwidth, const int dheight) const{
	...
	SDL_RenderCopy(g_renderer, m_texture, &srcRect, &dstRect);
}