...
NewGameScreen::NewGameScreen(ScreenState* currentState, int *boardCount, int *boardSize, int *lineSize, int *gameSeed, bool *manualMode, bool *writeLog, std::string *logName){
...
void NewGameScreen::init(TTF_Font* headlineFont, TTF_Font* guiFont, const unsigned int width, const unsigned int height){

	const unsigned int halfWidth = width / 2;
	const SDL_Color black = { 0, 0, 0, 255 };

	m_headlineText = new Text("New Game", headlineFont, black, 0, HEADLINE_PADDING);
	m_headlineText->setX(halfWidth - m_headlineText->getWidth() / 2);

	m_boardCountText = new Text("Board Count: ", guiFont, black, 0, m_headlineText->getY() + m_headlineText->getHeight() + HEADLINE_PADDING + PADDING_Y);
	m_boardSizeText = new Text("Board Size: ", guiFont, black, 0, m_boardCountText->getY());
	m_lineSizeText = new Text("Line Size: ", guiFont, black, 0, m_boardCountText->getY());

	m_boardCountTextField = new TextField(TextField::NUMBER, std::to_string(*m_boardCount), 0, m_boardCountText->getY(), 50, -1);
	m_boardSizeTextField = new TextField(TextField::NUMBER, std::to_string(*m_boardSize), 0, m_boardCountText->getY(), 30, 8);
	m_lineSizeTextField = new TextField(TextField::NUMBER, std::to_string(*m_lineSize), 0, m_boardCountText->getY(), 30, 8);
	...
	//
	m_gameSeedText = new Text("Game Seed: ", guiFont, black, 0, m_boardCountText->getY() + m_boardCountText->getHeight() + PADDING_Y);

	m_gameSeedTextField = new TextField(TextField::NUMBER, std::to_string(*m_gameSeed), 0, m_gameSeedText->getY(), 200, INT_MAX);
	...
	//
	m_manualModeText = new Text("Manual Mode:", guiFont, black, 0, m_gameSeedText->getY() + m_gameSeedText->getHeight() + PADDING_Y);
	...
	//
	m_writeLogText = new Text("Write log: ", guiFont, black, 0, m_manualModeText->getY() + m_manualModeText->getHeight() + PADDING_Y);
	...
	//
	m_logNameText = new Text("Log Name: ", guiFont, black, 0, m_writeLogText->getY() + m_writeLogText->getHeight() + PADDING_Y);

	m_logNameTextField = new TextField(TextField::TEXT, "log.txt", 0, m_logNameText->getY(), 200, -1);
	...
	//
	m_startGameButton = new Button("Start Game", 0, m_logNameText->getY() + m_logNameText->getHeight() + PADDING_Y);
	...
	//
	m_returnButton = new Button("Back to Main Menu", 0, 0);
...
void NewGameScreen::render(SDL_Renderer* renderer) const{ // The renderer will be used later
	...
	m_writeLogText->renderText();

	m_writeLogRadioButton->renderRadioButton();

	if (m_writeLogRadioButton->isChecked()){
		m_logNameText->renderText();
		m_logNameTextField->renderTextField(SDL_GetTicks());
	}

	m_headlineText->renderText();

	m_returnButton->renderButton();
}
