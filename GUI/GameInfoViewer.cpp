...
GameInfoViewer::GameInfoViewer(){
	m_headlineFont = TTF_OpenFont(g_NtttManager.FONT_PATH, (int)(g_NtttManager.FONT_SIZE * 1.1));
	if (m_headlineFont == NULL){
		std::cout << "Failed loading font: " << TTF_GetError() << std::endl;
		m_headlineFont = g_NtttManager.g_font;
	}

	m_movesFont = TTF_OpenFont(g_NtttManager.FONT_PATH, (int)(g_NtttManager.FONT_SIZE * 0.5));
	if (m_movesFont == NULL){
		std::cout << "Failed loading font: " << TTF_GetError() << std::endl;
...
void GameInfoViewer::renderGameInfoViewer() const{
	...
	if (m_boardCount != nullptr)
		m_boardCount->renderText();
	if (m_boardSize != nullptr)
		m_boardSize->renderText();
	if (m_lineSize != nullptr)
		m_lineSize->renderText();
	
	if (m_winnerText != nullptr)
